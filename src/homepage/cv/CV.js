  ...
  return (
    <>
      ...
      <div className="cv-container">
        {selectedLayout === 'left' && (
          <CvMainText selectedColor={selectedColor} userInfo={props.userInfo} />
        )}
        <div className="cv-right-container">
          {props.cvItems.includes('CvExperience') && (
            <CvExperience selectedColor={selectedColor} />
          )}
          {props.cvItems.includes('CvOtherExperience') && (
            <CvOtherExperience selectedColor={selectedColor} />
          )}
          {props.cvItems.includes('CvEducation') && (
            <CvEducation selectedColor={selectedColor} />
          )}
          {props.cvItems.includes('CvHobbies') && (
        ...
        <div
          className="cv-main-signature"
          style={
            selectedLayout === 'left' ? { right: '2vMin' } : { left: '2vMin' }
          }
        >
          {props.userInfo.company && (
            <> CV generated for the {props.userInfo.company} HR department.</>
          )}
        </div>
      </div>
    </>
